// Import necessary hooks
import { useNavigate, Outlet } from "react-router-dom";

// CSS modules
import NavbarStyle from "./Navbar.module.css";

// Material UI components for theme toggle switch
import { styled } from '@mui/material/styles';
import Switch from '@mui/material/Switch';

/**
 * Custom styled switch component for theme toggle
 * Creates a customized iOS-style switch with sun/moon icons
 */
const IOSSwitch = styled((props) => (
    <Switch
        focusVisibleClassName=".Mui-focusVisible"
        disableRipple
        {...props}
    />
))(({ theme }) => ({
    width: 62,
    height: 34,
    padding: 7,
    '& .MuiSwitch-switchBase': {
        margin: 1,
        padding: 0,
        transform: 'translateX(6px)',
        '&.Mui-checked': {
            color: '#fff',
            transform: 'translateX(22px)',
            '& .MuiSwitch-thumb:before': {
                // Moon icon for dark mode
                backgroundImage: `url('data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" height="20" width="20" viewBox="0 0 20 20"><path fill="${encodeURIComponent(
                    '#fff',
                )}" d="M4.2 2.5l-.7 1.8-1.8.7 1.8.7.7 1.8.6-1.8L6.7 5l-1.9-.7-.6-1.8zm15 8.3a6.7 6.7 0 11-6.6-6.6 5.8 5.8 0 006.6 6.6z"/></svg>')`,
            },
            '& + .MuiSwitch-track': {
                opacity: 1,
                // backgroundColor: theme.palette.mode === 'dark' ? '#8796A5' : '#aab4be',
                backgroundColor: theme.palette.mode === 'dark' ? '#2ECA45' : '#a75de1',
            },
        },
    },
    '& .MuiSwitch-thumb': {
        // backgroundColor: theme.palette.mode === 'dark' ? '#003892' : '#001e3c',
        backgroundColor: theme.palette.mode === 'light' ? '#323abb' : '#39393D',
        width: 32,
        height: 32,
        '&:before': {
            content: "''",
            position: 'absolute',
            width: '100%',
            height: '100%',
            left: 0,
            top: 0,
            backgroundRepeat: 'no-repeat',
            backgroundPosition: 'center',
            // Sun icon for light mode
            backgroundImage: `url('data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" height="20" width="20" viewBox="0 0 20 20"><path fill="${encodeURIComponent(
                '#fff',
            )}" d="M9.305 1.667V3.75h1.389V1.667h-1.39zm-4.707 1.95l-.982.982L5.09 6.072l.982-.982-1.473-1.473zm10.802 0L13.927 5.09l.982.982 1.473-1.473-.982-.982zM10 5.139a4.872 4.872 0 00-4.862 4.86A4.872 4.872 0 0010 14.862 4.872 4.872 0 0014.86 10 4.872 4.872 0 0010 5.139zm0 1.389A3.462 3.462 0 0113.471 10a3.462 3.462 0 01-3.473 3.472A3.462 3.462 0 016.527 10 3.462 3.462 0 0110 6.528zM1.665 9.305v1.39h2.083v-1.39H1.666zm14.583 0v1.39h2.084v-1.39h-2.084zM5.09 13.928L3.616 15.4l.982.982 1.473-1.473-.982-.982zm9.82 0l-.982.982 1.473 1.473.982-.982-1.473-1.473zM9.305 16.25v2.083h1.389V16.25h-1.39z"/></svg>')`,
        },
    },
    '& .MuiSwitch-track': {
        opacity: 1,
        backgroundColor: theme.palette.mode === 'dark' ? '#8796A5' : '#ffffff',
        // backgroundColor: theme.palette.mode === 'light' ? '#E9E9EA' : '#39393D',
        borderRadius: 20 / 2,
    },
}));

export default function Navbar(props) {
    // Navigation hook for programmatic routing
    const navigate = useNavigate();

    return (
        <>
            {/* Navigation bar container */}
            <div className={NavbarStyle?.navbar} style={props?.darkMode ? { backgroundColor: "#000000", color: "#ffffff" } : undefined}>
                {/* Logo section that navigates to home on click */}
                <div class={NavbarStyle?.navbarLogo} onClick={() => navigate("/")}>
                    <img src="/assets/logo.png" alt="logo" />
                    <span>PhotoFolio</span>
                </div>
                {/* Theme toggle switch */}
                <IOSSwitch
                    sx={{ m: 1 }}
                    checked={props?.darkMode}
                    onChange={(e) => {
                        props?.setDarkMode(!props?.darkMode) // Toggle dark mode state
                    }}
                    title={props?.darkMode ? "Dark Mode" : "Light Mode"} // Tooltip text
                />
            </div>

            {/* Outlet for nested routes */}
            <Outlet />
        </>
    )
}